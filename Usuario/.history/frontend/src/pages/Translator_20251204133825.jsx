...
function Translator() {
  ...
  const simulateHandDetection = useCallback(async () => {
    ...
    // 🔴 CORRECCIÓN: Generar landmarks con patrones REALES para "hola" y "gracias"
    const currentTime = Date.now();
    const isHolaPattern = Math.floor(currentTime / 4000) % 2 === 0; // Cambia cada 4 segundos
    ...
    if (isHolaPattern) {
      // 🔵 PATRÓN "HOLA" - Mano abierta, dedos extendidos
      console.log("🤚 Simulando: HOLA (mano abierta)");
      
      // Pulgar (índices 3-5)
      fakeLandmarks[3] = wristX - 0.15; fakeLandmarks[4] = wristY; fakeLandmarks[5] = wristZ;
      // Índice (6-8)
      fakeLandmarks[6] = wristX - 0.05; fakeLandmarks[7] = wristY - 0.2; fakeLandmarks[8] = wristZ;
      // Medio (9-11)
      fakeLandmarks[9] = wristX; fakeLandmarks[10] = wristY - 0.25; fakeLandmarks[11] = wristZ;
      // Anular (12-14)
      fakeLandmarks[12] = wristX + 0.05; fakeLandmarks[13] = wristY - 0.2; fakeLandmarks[14] = wristZ;
      // Meñique (15-17)
      fakeLandmarks[15] = wristX + 0.15; fakeLandmarks[16] = wristY - 0.1; fakeLandmarks[17] = wristZ;
      
      // Segundas falanges (simplificado)
      for (let i = 18; i < 63; i += 3) {
        const baseIndex = i - 15;
        fakeLandmarks[i] = fakeLandmarks[baseIndex] * 0.8;
        fakeLandmarks[i+1] = fakeLandmarks[baseIndex+1] * 0.9;
        fakeLandmarks[i+2] = fakeLandmarks[baseIndex+2] + 0.02;
      }
    } else {
      // 🟡 PATRÓN "GRACIAS" - Mano más cerrada, dedos doblados
      console.log("✊ Simulando: GRACIAS (mano cerrada)");
      
      // Todos los dedos más cerca y más bajos
      // Pulgar (3-5)
      fakeLandmarks[3] = wristX - 0.05; fakeLandmarks[4] = wristY - 0.05; fakeLandmarks[5] = wristZ;
      // Índice (6-8)
      fakeLandmarks[6] = wristX; fakeLandmarks[7] = wristY - 0.1; fakeLandmarks[8] = wristZ;
      // Medio (9-11)
      fakeLandmarks[9] = wristX; fakeLandmarks[10] = wristY - 0.12; fakeLandmarks[11] = wristZ;
      // Anular (12-14)
      fakeLandmarks[12] = wristX; fakeLandmarks[13] = wristY - 0.1; fakeLandmarks[14] = wristZ;
      // Meñique (15-17)
      fakeLandmarks[15] = wristX + 0.05; fakeLandmarks[16] = wristY - 0.05; fakeLandmarks[17] = wristZ;
      
      // Segundas falanges - más cerca aún (simulando puño)
      for (let i = 18; i < 63; i += 3) {
        const fingerGroup = Math.floor((i-18)/15);
        fakeLandmarks[i] = wristX + (fingerGroup - 2) * 0.02;
        fakeLandmarks[i+1] = wristY - 0.05 - (fingerGroup * 0.02);
        fakeLandmarks[i+2] = wristZ + 0.01;
      }
    }
